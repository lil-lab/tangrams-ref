...
# Results 3: Exponential relationship to image similarity
...
```{r}
## Compare with dichotomous model
# Create the dichotomous visual similarity variable 
# (using a median split to define 'High' vs 'Low' similarity)
shepard_same_thread$image_sim_dichotomous <- ifelse(
  shepard_same_thread$image_sim_c > median(shepard_same_thread$image_sim_c, na.rm = TRUE), 
  1, 0
)

# Model
m_dichot_s = brm(text_sim_c ~ image_sim_dichotomous + (1 + image_sim_dichotomous | player_id), 
                   data = shepard_same_thread, iter=10000, 
                   control = list(adapt_delta = 0.99, max_treedepth = 15))


loo_dichot_s <- loo(m_dichot_s)

# Compare the continuous quadratic model against the dichotomous model
loo_compare(loo_dichot_s, loo1_s,loo2_s,loo3_s)
```
...
### Post text sim vs visual diff to interactive phase, split by jaccard

```{r}
# Calculate median jaccard for the post phase
median_jaccard <- median(df$jaccard_index[df$phase == "post"], na.rm = TRUE)

# Create df_post with jaccard groups
df_post <- df %>%
  filter(phase == "post") %>%
  mutate(jaccard_group = if_else(jaccard_index <= median_jaccard, 
                                  "low Jaccard", 
                                  "high Jaccard"),
         jaccard_group = factor(jaccard_group, 
                               levels = c("low Jaccard", "high Jaccard")))

# Create the plot
df_post %>% 
  ggplot(aes(x = avg_sim_AB_interactive, y = similarity, color = jaccard_group)) +
  geom_jitter(width = .025, height = .025, alpha = .1) +
  geom_smooth(
    method = 'glm',
    formula = y ~ poly(x, 2),
  ...
  theme_few() +
  labs(x = "visual similarity to interactive phase", y = "description embedding similarity") +
  scale_x_continuous(breaks = seq(-1, 1, by = 0.2)) +
  scale_color_manual(values = c("low Jaccard" = "#E69F00", "high Jaccard" = "black")) +
  theme(
    aspect.ratio = 1,
    axis.text = element_text(size = 12),
    axis.title = element_text(size = 14),
...
#ggsave(here("analyses/img/text_by_image_sim_all_jaccard.pdf"))
```
...
# Create df_post with jaccard groups and filter for low jaccard only
df_post <- df %>%
  filter(phase == "post") %>%
  mutate(jaccard_group = if_else(jaccard_index <= median_jaccard, 
                                  "Low Jaccard", 
         ...
         jaccard_group = factor(jaccard_group, 
                               levels = c("Low Jaccard", "High Jaccard"))) %>%
  filter(jaccard_group == "Low Jaccard")
```

```{r}
invisible(capture.output({
m1_post_low <- brm(similarity ~ avg_sim_AB_interactive_c + 
                 (1 + avg_sim_AB_interactive_c | game), 
               data = df_post%>% filter(jaccard_group == "low Jaccard"))

m2_post_low <- brm(similarity ~ poly(avg_sim_AB_interactive_c, 2) + 
                 (1 + poly(avg_sim_AB_interactive_c, 2) | game), 
               data = df_post%>% filter(jaccard_group == "low Jaccard"))

m3_post_low <- brm(similarity ~ exp(avg_sim_AB_interactive_c) + 
                 (1 + exp(avg_sim_AB_interactive_c) | game), 
               data = df_post%>% filter(jaccard_group == "low Jaccard"))
m4_post_low <- brm(similarity ~ exp(avg_sim_AB_interactive_c) + poly(avg_sim_AB_interactive_c, 2) + (1 + exp(avg_sim_AB_interactive_c) + avg_sim_AB_interactive_c | game), 
               data = df_post%>% filter(jaccard_group == "low Jaccard"))
}))
```

```{r}
loo_post_low1 <- loo(m1_post_low)
loo_post_low2 <- loo(m2_post_low)
loo_post_low3 <- loo(m3_post_low)
loo_post_low4 <- loo(m4_post_low)

loo_compare(loo_post_low1, loo_post_low2, loo_post_low3, loo_post_low4)
```
...
# Functional form on raw pixel-level image similarity

```{r}
shp_pix <- read_csv(here('analyses/data/shepard_pairs_pixsim.csv'), show_col_types = F) %>%
  filter(is_same_thread) %>%
  mutate(pixel_sim_c = as.numeric(scale(pixel_sim, scale = FALSE)),
         text_sim_c  = as.numeric(scale(text_sim,  scale = FALSE)))

cor.test(shp_pix$pixel_sim, shp_pix$image_sim)

m1_pix = brm(text_sim_c ~ pixel_sim_c + (1 + pixel_sim_c | player_id), data = shp_pix, iter = 4000, control = list(adapt_delta = 0.99, max_treedepth = 15))
m2_pix = brm(text_sim_c ~ poly(pixel_sim_c, 2) + (1 + poly(pixel_sim_c, 2) | player_id), data = shp_pix, iter = 4000, control = list(adapt_delta = 0.99, max_treedepth = 15))
m3_pix = brm(text_sim_c ~ exp(pixel_sim_c) + (1 + exp(pixel_sim_c) | player_id), data = shp_pix, iter = 4000, control = list(adapt_delta = 0.99, max_treedepth = 15))
m4_pix = brm(text_sim_c ~ poly(pixel_sim_c, 2) + exp(pixel_sim_c) + (1 + exp(pixel_sim_c) + pixel_sim_c || player_id), data = shp_pix, iter = 4000, control = list(adapt_delta = 0.99, max_treedepth = 15))

loo_compare(loo(m1_pix), loo(m2_pix), loo(m3_pix), loo(m4_pix))
```

# Robustness: Fig 3B nonlinearity under equal similarity coverage

```{r}
shp <- read_csv(here('analyses/data/shepard_pairs.csv'), show_col_types = F) %>%
  filter(is_same_thread) %>%
  mutate(bin = cut(image_sim, breaks = c(0.3,0.4,0.5,0.6,0.7,0.8,1.0), include.lowest = TRUE))
print(table(shp$bin))

fit_cov <- function(d) {
  d <- d %>% mutate(image_sim_c = image_sim - mean(image_sim),
                    text_sim_c  = text_sim  - mean(text_sim))
  m1 <- brm(text_sim_c ~ image_sim_c + (1 + image_sim_c | player_id), data = d, iter = 4000, control = list(adapt_delta = 0.99, max_treedepth = 15))
  m2 <- brm(text_sim_c ~ poly(image_sim_c, 2) + (1 + image_sim_c | player_id), data = d, iter = 4000, control = list(adapt_delta = 0.99, max_treedepth = 15))
  m3 <- brm(text_sim_c ~ exp(image_sim_c) + (1 + image_sim_c | player_id), data = d, iter = 4000, control = list(adapt_delta = 0.99, max_treedepth = 15))
  print(loo_compare(loo(m1), loo(m2), loo(m3)))
  print(round(fixef(m2), 3))
}

# (A) central similarity range only
fit_cov(shp %>% filter(image_sim >= 0.4, image_sim <= 0.8))

# (B) equal-coverage resample (downsample each bin to the smallest)
set.seed(1)
fit_cov(shp %>% group_by(bin) %>% slice_sample(n = min(table(shp$bin))) %>% ungroup())
```
