...
Empirica.gameInit((game, treatment) => {
  ...
  var configName = BASE_CONFIGURATION_PATH + "exp2_train_and_test_" + (treatment.configIndex).toString() + ".json";
  ...
  for (let i = 0; i < gameConfig.length; i++) {
    ...
    round.set("chat", []);
    round.set("numTrials", gameConfig.length);
    ...
    // round.set("tangramsOrder", trialConfig["tangrams_order"]);
